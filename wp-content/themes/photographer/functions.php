...
/* ============================================================================================================================================= */
	...
	function pixelwars_theme_enqueue()
	{
		...
		wp_enqueue_style( 'roboto', '//fonts.googleapis.com/css?family=Roboto:500,900italic,900,400italic,700italic,300,700,500italic,300italic,400' . $pixelwars_subset, null, null );
...
/* ============================================================================================================================================= */
	...
	if ( function_exists( 'add_image_size' ) )
	{
		...
		add_image_size( 'pixelwars_theme_image_size_nullx500', null, 500, true ); // (portfolio-single thumbnail)
		add_image_size( 'pixelwars_theme_image_size_1600', 1600 ); // (portfolio-single medium)
		
		add_image_size( 'pixelwars_theme_image_size_3840', 3840 ); // Full-size Image Resizing = 4K - 8MP - 3840px (Default)
		add_image_size( 'pixelwars_theme_image_size_1920', 1920 ); // Full-size Image Resizing = 2K - 2MP - 1920px
...
/* ============================================================================================================================================= */
	...
	function pixelwars_theme_open_graph_protocol()
	{
		if ( is_singular() )
		{
			...
			if ( $og_image_url != "" )
			{
				echo "\n" . '<meta property="og:image" content="' . esc_url( $og_image_url ) . '">' . "\n";
			}
...
/* ============================================================================================================================================= */
	...
	function pixelwars_theme_seo_wp_head()
	{
		...
		if ( is_singular() )
		{
			...
			{
				echo "\n" . '<meta name="description" content="' . esc_attr( $my_seo_description ) . '">' . "\n";
			}
			
			
			if ( $my_seo_keywords != "" )
			{
				echo '<meta name="keywords" content="' . esc_attr( $my_seo_keywords ) . '">' . "\n";
			}
		}
		else
		{
	...
	function pixelwars_theme_custom_box_show_portfolio( $post )
	{
		?>
			...
			<p class="direct-url-wrap" style="<?php if ( $pf_type == 'Direct URL' ) { echo 'display: block;'; } else { echo 'display: none;'; } ?>">
				...
				<input type="text" id="pf_direct_url" name="pf_direct_url" class="widefat code2" placeholder="Link Url" value="<?php echo esc_url( $pf_direct_url ); ?>">
...
/* ============================================================================================================================================= */
	...
	function slider( $atts, $content = "" )
	{
		...
		$slider = '<div class="owl-carousel owl-loading" data-items="' . $items . '" data-loop="' . $loop . '" data-center="' . $center . '" data-mouse-drag="' . $mouse_drag . '" data-nav="' . $nav . '" data-dots="' . $dots . '" data-autoplay="' . $autoplay . '" data-autoplay-speed="' . $speed . '" data-autoplay-timeout="' . $timeout . '">' . do_shortcode( $content ) . '</div>';
...
/* ============================================================================================================================================= */
	...
	function photo_wall( $atts, $content = "" )
	{
		...
		$args = array( 'post_type' => 'portfolio', 'orderby' => 'rand', 'posts_per_page' => -1 );
		...
		if ( $loop->have_posts() ) :
			...
			while ( $loop->have_posts() ) : $loop->the_post();
				...
				if ( preg_match( '/\[gallery.*ids=.(.*).\]/', $post_content, $ids ) )
				{
					...
					foreach ( $ids_in_array as $item_id )
					{
						$image = wp_get_attachment_image_src( $item_id, 'thumbnail' );
						
						if ( $image[0] != "" )
						{
							if ( $items_count <= 100 )
							{
								$images .= '<li><a href="#"><img alt="" src="' . $image[0] . '"></a></li>';
			...
			if ( $images != 'No gallery.' )
			{
				$output = '<div class="ri-grid" data-animation="' . $animation . '" data-interval="' . $interval . '" data-max-step="' . $max_step . '"><ul>' . $images . '</ul></div>';
			}
			else
			{
				$output = 'No gallery.';
...
/* ============================================================================================================================================= */
	...
	function pixelwars__portfolio_single_photo_gallery( $atts )
	{
			...
			$output = '<div class="media-grid-wrap">';
				...
				$output .= '<div class="pw-gallery pw-collage pw-collage-loading" data-gallery-style="' . esc_attr( $gallery_style ) . '" data-share="' . esc_attr( $gallery_share_button ) . '" data-fullscreen="' . esc_attr( $gallery_fullscreen_button ) . '" data-bg-opacity="' . esc_attr( $gallery_background_color_opacity ) . '" data-row-height="' . esc_attr( $gallery_row_height ) . '" data-effect="' . esc_attr( $gallery_image_load_effect ) . '">';
					...
					foreach ( $ids_array as $id_no )
					{
						...
						if ( $pixelwars_full_size_image_resizing == '2k' )
						{
							$attachment_image_full = wp_get_attachment_image_src( $attachment_id, 'pixelwars_theme_image_size_1920' );
						}
						elseif ( $pixelwars_full_size_image_resizing == 'medium' )
						{
							$attachment_image_full = wp_get_attachment_image_src( $attachment_id, 'pixelwars_theme_image_size_1400' );
						}
						elseif ( $pixelwars_full_size_image_resizing == 'no_resizing' )
						{
							$attachment_image_full = wp_get_attachment_image_src( $attachment_id, 'full' );
						}
						else
						{
							$attachment_image_full = wp_get_attachment_image_src( $attachment_id, 'pixelwars_theme_image_size_3840' );
						}
						
						
						$attachment_image_medium = wp_get_attachment_image_src( $attachment_id, 'pixelwars_theme_image_size_1600' );
						$attachment_image_thumbnail = wp_get_attachment_image_src( $attachment_id, 'pixelwars_theme_image_size_nullx500' );
						
						$attachment_image_alt = get_post_meta( $attachment_id, '_wp_attachment_image_alt', true );
						$attachment_image_caption = $wpdb->get_var( $wpdb->prepare( "SELECT post_excerpt FROM $wpdb->posts WHERE ID = %s", $attachment_id ) );
						
						$attachment_page = get_attachment_link( $attachment_id );
						
						$output .= '<a href="' . esc_url( $attachment_image_full[0] ) . '" data-size="' . esc_attr( $attachment_image_full[1] ) . 'x' . esc_attr( $attachment_image_full[2] ) . '" data-med="' . esc_url( $attachment_image_medium[0] ) . '" data-med-size="' . esc_attr( $attachment_image_medium[1] ) . 'x' . esc_attr( $attachment_image_medium[2] ) . '" data-attachment-page="' . $attachment_page . '">';
							
							$output .= '<img alt="' . $attachment_image_alt . '" src="' . esc_url( $attachment_image_thumbnail[0] ) . '" width="' . esc_attr( $attachment_image_thumbnail[1] ) . '" height="' . esc_attr( $attachment_image_thumbnail[2] ) . '">';
							
							if ( $attachment_image_caption != "" )
							{
								$output .= '<figure>'. $attachment_image_caption . '</figure>';
							}
	...
	function pixelwars__portfolio_single_photo_gallery_2( $atts )
	{
		extract( shortcode_atts( array( 'ids' => "" ), $atts ) );
		...
		if ( $ids_with_commas != "" )
		{
			$output = '<div class="media-grid-wrap">';
				...
				$output .= '<div class="mfp-gallery pw-collage pw-collage-loading" data-row-height="' . esc_attr( $gallery_row_height ) . '" data-effect="' . esc_attr( $gallery_image_load_effect ) . '">';
					...
					foreach ( $ids_array as $id_no )
					{
						...
						if ( $attachment_image_caption != "" )
						{
							$title_out = 'title="' . $attachment_image_caption . '"';
						}
						
						
						$output .= '<a href="' . esc_url( $attachment_image_full[0] ) . '" ' . $title_out . '>';
							
							$output .= '<img alt="' . $attachment_image_alt . '" src="' . esc_url( $attachment_image_thumbnail[0] ) . '">';
	...
	function pixelwars__homepage_photowall( $atts )
	{
		extract( shortcode_atts( array( 'ids' => "" ), $atts ) );
		...
		if ( $items_with_commas != "" )
		{
			...
			$output = '<div class="ri-grid" data-animation="' . $animation . '" data-interval="' . $interval . '" data-max-step="' . $max_step . '"><ul>';
				...
				foreach ( $items_in_array as $item )
				{
					...
					$output .= '<li><a href="#"><img alt="' . $image_alt . '" src="' . $image[0] . '"></a></li>';
				}
	...
	function pixelwars__homepage_landing_photowall( $atts )
	{
		extract( shortcode_atts( array( 'ids' => "" ), $atts ) );
		...
		if ( $items_with_commas != "" )
		{
			...
			$output = '<div class="ri-grid" data-animation="' . $animation . '" data-interval="' . $interval . '" data-max-step="' . $max_step . '"><ul>';
				...
				{
					$image_alt = get_post_meta( $item, '_wp_attachment_image_alt', true );
					$image = wp_get_attachment_image_src( $item, 'medium' );
	...
	function pixelwars__homepage_alternate_ken_slider( $atts )
	{
		...
		if ( $items_with_commas != "" )
		{
			...
			$output = '<ul class="ken-slider" data-animation="' . $animation . '" data-speed="' . $speed . '">';
				...
				foreach ( $items_in_array as $item )
				{
					...
					$output .= '<li><img alt="' . $image_alt . '" src="' . $image[0] . '"></li>';
				}
	...
	function pixelwars__homepage_alternate_2_owl_carousel( $atts )
	{
		extract( shortcode_atts( array( 'ids'  => "",
										'size' => 'thumbnail' ), $atts ) );
		...
		if ( $items_with_commas != "" )
		{
			...
			$output = '<div class="owl-carousel owl-loading" data-items="' . $items . '" data-loop="' . $loop . '" data-center="' . $center . '" data-mouse-drag="' . $mouse_drag . '" data-nav="' . $nav_links . '" data-dots="' . $nav_dots . '" data-autoplay="' . $autoplay . '" data-autoplay-speed="' . $autoplay_speed . '" data-autoplay-timeout="' . $autoplay_timeout . '">';
				...
				foreach ( $items_in_array as $item )
				{
					...
					$output .= '<div><img alt="" src="' . $image[0] . '"></div>';
				}
	...
	function pixelwars_gallery_shortcode( $output = "", $atts, $content = false, $tag = false )
	{
		$new_output = $output;
		...
		if ( is_singular( 'portfolio' ) && ( $pf_type == 'Photo Gallery' ) )
		{
			$new_output = pixelwars__portfolio_single_photo_gallery( $atts );
		}
		elseif ( is_singular( 'portfolio' ) && ( $pf_type == 'Photo Gallery 2' ) )
		{
			$new_output = pixelwars__portfolio_single_photo_gallery_2( $atts );
		}
		elseif ( is_page_template( 'template-homepage.php' ) )
		{
			$new_output = pixelwars__homepage_photowall( $atts );
		}
		elseif ( is_page_template( 'template-homepage_landing.php' ) )
		{
			$new_output = pixelwars__homepage_landing_photowall( $atts );
		}
		elseif ( is_page_template( 'template-homepage_alternate.php' ) )
		{
			$new_output = pixelwars__homepage_alternate_ken_slider( $atts );
		}
		elseif ( is_page_template( 'template-homepage_alternate_2.php' ) )
		{
			$new_output = pixelwars__homepage_alternate_2_owl_carousel( $atts );
		}
	...
	add_filter( 'post_gallery', 'pixelwars_gallery_shortcode', 10, 4 );
